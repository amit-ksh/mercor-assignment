...
const Section1 = () => {
    ...
    useEffect(() => {
        function revealText() {
            ...
            const words: NodeListOf<HTMLSpanElement> = textReavelContainer.current.querySelectorAll(`p span`)
            words.forEach((word, idx) => {
                if (idx < nWordsToReveal)
    ...
    return (
        <div ref={textReavelContainer} className='textreveal_wrapper'>
            <div className="textreveal_container">
                <p>
                    {text.split(' ').map((word, idx) => <Span key={idx} children={word + ' '} />)}
                </p>
            </div>
        </div>
    )
